...
export default RoyalCaninStock = () => {
    const [stock,setStock] = useState([])
    const [loading,setLoading] = useState(true)

    const retriveData = async () => {
        try{
            const response = await fetch("http://192.168.0.249:3000/royalCanin",{method:"GET"})
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View>
                <Text style={styles.title}>Royal Canin</Text>
                <ScrollView horizontal={true} >
                    <View>
                        <RowTab height={35} width={165} backgroundColor={"black"} content={["Codigo","Descripcion","Costo","Venta"]} text_color={"white"} text_weight={"bold"}></RowTab> 
                        <ScrollView>
                            {stock.map((product) => (
